...
export default class MyLianWenList extends NavigationPage {
 ...
 onBackAndroid = () => {
   ...
   ToastAndroid.show(i18n.t('MainMenu.press_Exit'),ToastAndroid.SHORT);
   //BackHandler.exitApp();
   return true;
 };
 ...
 _itemPress(item){
          ...
          MainLogic.commonSetRpcAndVnode(item.subChainAddress,item.rpcIp).then(data=>{
           this.setState({showLoading:false});
            if(data.isSuccess==0){
             ...
             Toast.success(i18n.t('MainMenu.backup_node_connected'));
             item.rpcIp = data.rpcIp;
             this.navigator.push({view: <LMain  BoardList={item}/>});
            }
            else if(data.isSuccess==3){
             Toast.success(i18n.t('MainMenu.backup_remote_sever_connected'));
             item.rpcIp = data.rpcIp;
             this.navigator.push({view: <LMain  BoardList={item}/>});
            }
             ...
             Toast.fail(i18n.t('MainMenu.Exception_obtainChainInfo'));
            }
          })
 }
   ...
   renderPage() {
       ...
       return (
           ...
           <View style={{height: 2}} />
           <ListRow  title={i18n.t('MainMenu.what_is_interested')} icon={require('../styles/menu/wen.png')}  titleStyle={{color:'#00A29A',fontSize:18}} />
           {rows}
